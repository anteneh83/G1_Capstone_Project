...
export default function DestinationList() {
  ...
  return (
    <section className="my-10 max-w-[1200px] mx-auto relative p-4 top-25">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative ">
      {loading ? (
        <p className="p-4 animate-pulse ">Fetching destinations...</p>
      ) : (
        filteredDestinations.map((dest) => (
          <div key={dest.id} className="bg-white shadow-md rounded-lg p-5">
            <h3 className="text-xl font-semibold">{dest.name}</h3>
            <p className="text-gray-600">Location: {dest.location}</p>
            <p className="text-gray-600">Price: ${dest.price}</p>
